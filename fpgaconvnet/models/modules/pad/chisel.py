...
try:
    DEFAULT_PAD_RSC_MODELS: dict[str, ResourceModel] = { rsc_type: get_cached_resource_model(PadChisel,
                                    rsc_type, "default") for rsc_type in DEFAULT_CHISEL_PLATFORM.resource_types }
except FileNotFoundError:
    print("CRITICAL WARNING: default resource models not found for Pad, default resource modelling will fail")

@eval_resource_model.register
def _(m: PadChisel, rsc_type: str, _model: Optional[ResourceModel] = None) -> int:

    # get the resource model
    model: ResourceModel = _model if _model is not None else DEFAULT_PAD_RSC_MODELS[rsc_type]
