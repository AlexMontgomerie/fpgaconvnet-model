...
class EltWiseLayer3D(MultiPortLayer3D):
    ...
    def __post_init__(self):
        ...
        # init modules
        self.modules = {
            "eltwise3d" : EltWise3D(self.rows_in(), self.cols_in(), self.depth_in(), self.channels_in()//self.coarse, self.ports_in, eltwise_type=self.op_type, broadcast=self.broadcast, backend=self.backend, regression_model=self.regression_model),
        }
