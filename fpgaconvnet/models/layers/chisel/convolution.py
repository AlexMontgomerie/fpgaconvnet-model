...
def initialise_modules(self):
    ...
    self.modules["accum"] = Accum(self.rows_out(), self.cols_out(),
            int(self.kernel_size[0]*self.kernel_size[1]*self.channels_in()/(
                self.fine*self.coarse_in)),
    ...
    # re-initialise modules
    for m in self.modules:
        self.modules[m].backend = self.backend
        self.modules[m].__post_init__()
