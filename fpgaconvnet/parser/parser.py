    ...
    def add_split(self, graph, dimensionality):
        ...
        for node in nodes:
            ...
            if len(nodes_out) > 1 and not graph.nodes[node]['type'] == LAYER_TYPE.Chop:
                ...
                # create split node hardware config
                config = {
                    "rows": graph.nodes[node]['hw'].rows_out(),
                    "cols": graph.nodes[node]['hw'].cols_out(),
                    "channels": graph.nodes[node]['hw'].channels_out(),
                    "coarse": graph.nodes[node]['hw'].streams_out(),
                    "ports": len(nodes_out),
                    "data_t": graph.nodes[node]['hw'].data_t.to_dict(),
